...
pub fn list_latest_active_logs(limit: usize) -> anyhow::Result<()> {
    use std::fs;
...
fn collect_md_files(dir: &std::path::Path, entries: &mut Vec<LogEntry>) -> anyhow::Result<()> {
    use crate::utils::load_entry_content;
    use std::fs;
...
fn extract_log_info(content: &str, path: &std::path::Path) -> anyhow::Result<LogEntry> {
    use crate::md_frontmatter::extract_frontmatter;
    use crate::{HypothesisLog, LiteratureLog, KnowledgeLog};
